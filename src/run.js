...
for (const eventHandler of eventHandlers)
    client.on(eventHandler.eventName, eventHandler.run);
client.on('ready', () => {
...
client.login(config.BOT_TOKEN).then(async () => {
    if (client.user?.tag)
        console.log(`Logged in as ${client.user.tag}. Getting ready...`);
    else
        console.log('Logged in. Getting ready...');
    const [commandArg] = process.argv.slice(2);
    if (commandArg === 'register-slash') {
        const slashCommandStructures = slashCommandHandlerMap.map((command) => command.structure);
        await Promise.all(client.guilds.cache.map((guild) => guild.commands.set(slashCommandStructures)));
