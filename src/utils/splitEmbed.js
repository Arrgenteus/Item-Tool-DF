import { MAX_EMBED_DESC_LENGTH, MAX_EMBED_FOOTER_LENGTH, MAX_SPLIT_EMBED_DESC_LENGTH, } from './constants';
export default class SplitEmbed {
    ...
    titleText = '';
    footerText = '';
    constructor(delimiter, maxEmbeds) {
    ...
    addText(text) {
        ...
        let separatorIndex = text.length <= remainingLength
            ? remainingLength
        ...
        if (remainingText.length) {
            ...
            if (this.footerText)
                this.setFooter(this.footerText);
        }
    }
    setTitle(text) {
        if (text.length > 256)
            throw new RangeError('Embed title cannot exceed 256 characters');
        this.footerText = text;
        this.firstEmbed.title = text;
    }
    setFooter(text) {
        if (text.length > MAX_EMBED_FOOTER_LENGTH)
            throw new RangeError(`Embed footer cannot exceed ${MAX_EMBED_FOOTER_LENGTH} characters`);
        this.footerText = text;
