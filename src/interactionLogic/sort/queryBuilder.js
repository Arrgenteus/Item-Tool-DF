...
function getItemTypeFilter(itemType) {
    if (itemType === 'weapon')
...
export async function getSortQueryPipeline({ itemType, ascending, sortExpression, weaponElement, minLevel, maxLevel, charID, excludeTags, nextPageValueLimit, prevPageValueLimit, }, returnShortResult = false) {
    ...
    const tagsToExclude = ['ak', 'alexander', 'cosmetic'];
    for (const tag of excludeTags ?? []) {
        if (tag === 'none')
            tagsToExclude.push([]);
        else
            tagsToExclude.push(tag);
    }
    ...
    if (nextPageValueLimit !== undefined) {
        ...
        if (ascending)
            secondaryFilter.customSortValue.$gt = nextPageValueLimit;
        else
            secondaryFilter.customSortValue.$lt = nextPageValueLimit;
    }
    else if (prevPageValueLimit !== undefined) {
        secondaryFilter.customSortValue = {};
        if (ascending)
            secondaryFilter.customSortValue.$lt = prevPageValueLimit;
        else
            secondaryFilter.customSortValue.$gt = prevPageValueLimit;
    }
    const sortOrder = (ascending && !prevPageValueLimit) || (!ascending && prevPageValueLimit) ? 1 : -1;
    return [
