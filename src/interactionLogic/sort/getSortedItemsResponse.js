...
import { PRETTY_ITEM_TYPES, PRETTY_TAG_NAMES, PRETTY_TO_BASE_ITEM_TYPE, } from '../../utils/itemTypeData';
import { QUERY_RESULT_LIMIT, QUERY_SHORT_RESULT_LIMIT, SORTABLE_TAGS, SORT_ACTIONS, } from './constants';
import { ValidationError } from '../../errors';
const ITEM_LIST_DELIMITER = ', `';
const itemCollection = dbConnection.then((db) => db.collection(config.DB_COLLECTION));
const allValidResists = itemCollection.then((collection) => collection.aggregate([
    { $unwind: '$resists' },
    { $group: { _id: null, keys: { $addToSet: '$resists.k' } } },
]));
function getFiltersUsedText({ ascending, weaponElement, charID, minLevel, maxLevel, }) {
    const filterText = [];
    if (weaponElement)
        filterText.push(`**Weapon Element:** ${capitalize(weaponElement)}`);
    const levelFilterText = [];
    if (ascending)
        filterText.push('**Order:** Ascending');
    if (charID) {
        levelFilterText.push(`**Char ID:** [${charID}](https://account.dragonfable.com/CharPage?id=${charID})`);
    }
    ...
    if (levelFilterText.length)
        filterText.push(levelFilterText.join(', '));
    return filterText.join('\n');
}
function getTagFilterDropDownComponent(excludeTags) {
    return [
        {
            type: 'ACTION_ROW',
            components: [
...
function getFullResultsButtonComponent(itemType, excludeTags) {
    const excludeTagsList = (excludeTags ? [...excludeTags] : []).join(',');
    return [
        {
            type: 'ACTION_ROW',
            components: [
                {
                    ...
                    customId: [SORT_ACTIONS.SHOW_RESULTS, itemType, excludeTagsList].join(INTERACTION_ID_ARG_SEPARATOR),
                    style: 'PRIMARY',
                },
            ],
        },
...
function getPrevAndNextSortPageNavigationComponents(excludeTags, prevPageValueLimit, nextPageValueLimit) {
    const excludeTagsList = (excludeTags ? [...excludeTags] : []).join(',');
    ...
    if (prevPageValueLimit !== undefined) {
        buttonComponents.push({
            type: 'BUTTON',
            label: '\u276e Prev Page',
            customId: [SORT_ACTIONS.PREV_PAGE, prevPageValueLimit, excludeTagsList].join(INTERACTION_ID_ARG_SEPARATOR),
            style: 'PRIMARY',
        });
    }
    if (nextPageValueLimit !== undefined) {
        buttonComponents.push({
            type: 'BUTTON',
            label: 'Next Page \u276f',
            customId: [SORT_ACTIONS.NEXT_PAGE, nextPageValueLimit, excludeTagsList].join(INTERACTION_ID_ARG_SEPARATOR),
            style: 'PRIMARY',
        });
    }
...
function itemButtonList(excludeTags) {
    const excludeTagsList = (excludeTags ? [...excludeTags] : []).join(',');
    ...
    return itemTypeList.map((itemTypeSubset) => ({
        type: 'ACTION_ROW',
        components: itemTypeSubset.map((itemType) => ({
            type: 'BUTTON',
...
function getAllItemDisplayMessage(sortFilterParams) {
    return {
        embeds: [
            {
                title: `Sort items by ${sortFilterParams.sortExpression.pretty}`,
                description: `${getFiltersUsedText(sortFilterParams)}\n\nClick on one of the buttons below`,
            },
        ],
        components: getTagFilterDropDownComponent(sortFilterParams.excludeTags).concat(itemButtonList(sortFilterParams.excludeTags)),
    };
}
export async function getSortedItemListMessage(sortFilterParams, returnShortResult) {
    ...
    const sortResults = (await itemCollection).aggregate(pipeline);
    let itemGroup = null;
    let sortedList = '';
    let lastResult = '';
    let groupCount = 0;
    ...
    // being redundant
    const queryResultLimit = returnShortResult
    ...
    while (itemGroup !== null && groupCount < queryResultLimit - 1) {
        ...
        const itemDisplayList = items.map((item) => {
            let possibleTags = item.variant_info
    ...
    if (!sortedList && lastResult) {
        const ellipses = ' **...**';
        const lastItemIndexBeforeLimit = lastResult
    ...
    if (returnShortResult) {
        buttonRow = getFullResultsButtonComponent(sortFilterParams.itemType, sortFilterParams.excludeTags);
    }
    else {
        buttonRow = getPrevAndNextSortPageNavigationComponents(sortFilterParams.excludeTags, 
    ...
    const title = `Sort ${PRETTY_ITEM_TYPES[sortFilterParams.itemType]} by ${sortFilterParams.sortExpression.pretty}`;
    // Display filters used in the first embed
    const filters = getFiltersUsedText(sortFilterParams);
    return {
...
export async function getSortResultsMessage(itemTypeOption, sortFilterParams, returnShortResult = false) {
    ...
    return getSortedItemListMessage({ ...sortFilterParams, itemType: itemTypeOption }, returnShortResult);
}
export async function getSortResultsMessageUsingMessageFilters(embedTitle, embedDesc, selectedTagsToExclude, itemType, valueLimit) {
    const sortExpressionMatch = embedTitle.match(/^Sort ([a-z/]+?) by (.+)$/i);
    const itemTypeMatch = sortExpressionMatch[1];
    if (!itemType)
        itemType = PRETTY_TO_BASE_ITEM_TYPE[itemTypeMatch] || 'items';
    const sortExpressionInput = sortExpressionMatch[2];
    const sortExpression = parseSortExpression(sortExpressionInput, true);
    let excludeTags;
    ...
    if (embedDesc) {
        if (itemType === 'weapon') {
            const [, weaponElement] = embedDesc.match(/\*\*Weapon Element:\*\* ([a-z0-9?]+)/i) || [];
            sortFilterParams.weaponElement = weaponElement?.toLowerCase();
        }
        const [, charID] = embedDesc.match(/\*\*Char ID:\*\* \[(\d+)\]/i) || [];
        const [, minLevel] = embedDesc.match(/\*\*Min Level:\*\* (\d+)/i) || [];
        const [, maxLevel] = embedDesc.match(/\*\*Max Level:\*\* (\d+)/i) || [];
        const ascending = !!embedDesc.match(/\*\*Order:\*\* Ascending/);
