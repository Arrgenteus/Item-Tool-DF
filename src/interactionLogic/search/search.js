...
import { formatQueryResponse, getButtonForMoreItemImages, getButtonListForSimilarResults, } from './formattedResults';
import { getIndexNames, getVariantAndUnaliasTokens, romanIntToInt } from './utils';
export function getSpecificCategoryFilterQuery(itemSearchCategory) {
    if (itemSearchCategory === 'cosmetic') {
        return { terms: { common_tags: ['cosmetic'] } };
    }
    if (['weapon', 'accessory', 'item', 'pet'].includes(itemSearchCategory))
        return undefined;
    let itemTypeFilterItems;
...
function getMaxAndMinLevelFilter({ maxLevel, minLevel, }) {
    let levelFilter;
    ...
    if (minLevel !== undefined) {
        if (levelFilter)
            levelFilter.range.level.gte = minLevel;
        else
            levelFilter = { range: { level: { gte: minLevel } } };
    }
    return levelFilter;
}
function getMatchQueryBody(tokens, itemSearchCategory, familyTitles = false) {
    ...
    const minimumShouldMatch = isPet
        ? `${2 + stopWordCount}<75%`
        : `${3 + stopWordCount}<75%`;
...
export async function fetchAutocompleteItemResults({ term, itemSearchCategory, maxLevel, minLevel, }) {
    const query = {
        bool: {
    ...
    const itemIndexes = getIndexNames(itemSearchCategory);
    const { unaliasedTokens } = getVariantAndUnaliasTokens(term, itemSearchCategory);
    const levelFilter = getMaxAndMinLevelFilter({ maxLevel, minLevel });
    ...
    return responseBody.aggregations.grouped_titles.buckets.map((result) => ({
        name: result.key,
        value: result.key,
    }));
}
export async function fetchItemSearchResult({ term, itemSearchCategory, maxLevel, minLevel, }) {
    const query = {
        bool: {
            filter: getSpecificCategoryFilterQuery(itemSearchCategory),
            minimum_should_match: 1,
    ...
    const itemIndexes = getIndexNames(itemSearchCategory);
    const { unaliasedTokens, variantRomanNumber } = getVariantAndUnaliasTokens(term, itemSearchCategory);
    let variantFilter;
    ...
    const searchQuery = {
        ...
        body: {
            ...
            post_filter: levelFilter || variantFilter
                ? {
                    bool: {
                        filter: [levelFilter, variantFilter].filter((filterType) => !!filterType),
                    },
                }
                : undefined,
            aggs: {
...
export async function getSearchResultMessagewithButtons({ term, itemSearchCategory, userIdForSimilarResults, maxLevel, minLevel, }) {
    ...
    const searchResultImageCount = searchResultResponseBody.hits.hits[0]._source.images?.length ?? 0;
    if (searchResultImageCount > 1) {
        const searchResultTitle = searchResultResponseBody.hits.hits[0]._source.full_title;
...
export async function getSearchResultMessage({ term, itemSearchCategory, maxLevel, minLevel, }) {
    const searchResultResponseBody = await fetchItemSearchResult({
        term,
        itemSearchCategory,
        maxLevel,
...
export async function getAllItemImages({ itemName, itemSearchCategory, maxLevel, minLevel, }) {
    const itemIndexes = getIndexNames(itemSearchCategory);
    const filterQuery = {
        bool: {
            filter: [
    ...
    return itemImages.slice(0, 10).map((imageLink) => ({ image: { url: imageLink } }));
}
